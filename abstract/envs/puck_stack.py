...
class PuckStack:
    ...
    def __init__(self, num_pucks=2, num_blocks=8, block_size=28, show_pick=False,
                 show_place=False, min_radius=8, max_radius=10, num_redundant_pucks=0):

        assert num_pucks + num_redundant_pucks <= num_blocks ** 2
        ...
        self.num_redundant_pucks = num_redundant_pucks

        self.observation_space = spaces.Tuple(
            [spaces.Box(np.zeros([self.gridSize, self.gridSize, 1]), np.ones([self.gridSize, self.gridSize, 1]), dtype=np.float32),
             spaces.Discrete(2)])
        self.holdingImage = []
        self.state = None
        self.max_episode = num_pucks * self.MAX_EPISODE_LENGTH_MULTIPLIER
    ...
    def reset(self):
        ...
        for i in range(self.num_pucks + self.num_redundant_pucks):
            while True:
    ...
    def step(self, action):
        ...
        if self.state[1] == 0:  # if both objs on the grid
            if np.any(self.state[0] == self.num_pucks):
                reward = 10
                done = 1

...
from ....envs.puck_stack_subgoal import PuckStackSubgoal
...
from ....abstraction.graph_option import GraphOption
...
def run_task(args, previous_agent=None, previous_keys=None, previous_experience=None, previous_dqn=None):
    ...
    env = PuckStackSubgoal(args.task, num_blocks=args.num_blocks, min_radius=args.min_radius,
                           max_radius=args.max_radius)

    graph_option = None
    if previous_agent is not None:
        previous_agent.env = env
        graph_option = GraphOption(previous_agent.graph, learning_rate=args.option_learning_rate)
    ...
    while num_act_steps > 0:
        ...
        if previous_agent is not None:
            ...
            rewards, _ = abstraction_utils.collect_experience_q_option(
                env, act, get_block, buffers, option, graph_option, 0.9, args.option_epsilon,
                max_buffer_size=args.max_buffer_size, num_episodes=args.num_start_episodes, verbose=args.verbose,
                keys=keys, task=args.task, saver=args.saver, remember=remember
            )
        else:
            rewards, _ = abstraction_utils.collect_experience(
                ...
                verbose=args.verbose, keys=keys, task=args.task, saver=args.saver
            )
    ...
    for i in range(num_act_steps):
        ...
        goal = homo.partition.get_goal_state_action_block(task_idx=args.task)
...
def main(args):
    ...
    if args.gifs_dir is not None:
        saver = vis_utils.Saver(max_value=3, dir_path=args.gifs_dir)
    ...
    if args.reversed:
        tasks = [PuckStackSubgoal.TASK_STACK_2_AND_2, PuckStackSubgoal.TASK_STACK_3]
    else:
        tasks = [PuckStackSubgoal.TASK_STACK_3, PuckStackSubgoal.TASK_STACK_2_AND_2]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reversed", default=False, action="store_true", help="go from task b to task a")
    ...
    parser.add_argument("--option-learning-rate", type=float, help="learning rate for option q-value updated with MC")
    ...
    parser.add_argument("--min-radius", type=int, default=5, help="minimum puck radius")
    ...
    parser.add_argument("--option-epsilon", type=float, default=0.1, help="exploration fraction for the option policy")
